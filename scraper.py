...
class Scraper:
    ...
    def _find_date(self, soup):
        ...
        Returns date (string)
        '''
        text = soup.find(id='display-date').text
	...
	#models = ['camry', 'civic', 'corolla', 'accord']
        ...
        for row in rows:
            ...
            try:
                ...
                tag = row.find("span", class_="pl").next_sibling.next_sibling
		#.next_sibling.next_sibling
                title = tag.text

                tag = row.find("span", class_="pl")
		...
		if not model:
	  	    model = self._find_model(body, models)
    ...
    def load(self):
        ...
        '''
        self.df = sql.read_sql("SELECT * FROM " + self.table_name, self.conn)
